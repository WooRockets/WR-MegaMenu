...
if ( ! class_exists( 'WR_Megamenu_Product_Addons' ) ) :
 ...
 * @since    1.0.0
 */
class WR_Megamenu_Product_Addons extends WR_Megamenu_Product_Info {
	/**
		...
		if ( 'uninstall' == $action ) {
			...
			if ( 'authenticate' != $action ) {
				...
				if ( isset( $_POST['remember'] ) && (boolean) $_POST['remember'] ) {
					update_option( 'wr_mm_customer_account', array( 'username' => $username, 'password' => $password ) );
				}
			}
		}
